...
class KalshiClient:
    ...
    def iter_events(
        ...
        """Iterate individual markets via the events endpoint (no parlays).

        Uses with_nested_markets=true so each yielded Market belongs to a
        real single-leg event rather than a multi-variate parlay collection.
        """
        while True:
            params: dict = {"limit": limit, "status": status, "with_nested_markets": "true"}
            if cursor:
                params["cursor"] = cursor
            ...
            for event in data.get("events", []):
                ...
                for raw in event.get("markets", []):
                    raw.setdefault("event_title", event_title)
                    markets.append(Market.from_dict(raw))
